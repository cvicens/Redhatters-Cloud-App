...
function init(server) {
    ...
    var updateUserList = function() {
        io.sockets.in(SHARED_ROOM).emit('userlist', {
        "USERS": USERS
        });
    };
    ...
    io.sockets.on('connection', function(socket) {
        ...
        /*socket.on('add-question', function(data) {
            ...
            io.sockets.in(SHARED_ROOM).emit('question', message);
        });*/
        ...
        socket.on(NEXT_QUESTION_MESSAGE, function(data) {
            ...
            if (typeof currentQuiz !== 'undefined' && 
                ...
                if (currentQuiz.currentQuestionIndex < currentQuiz.quiz.questions.length - 1) {
                    ...
                    io.sockets.in(liveQuizId).emit(NEW_QUESTION_MESSAGE, { // room was SHARED before
                    ...
                    if (currentQuiz.currentQuestionIndex >= currentQuiz.quiz.questions.length - 1) {
                        io.sockets.in(liveQuizId).emit(LAST_QUESTION_MESSAGE, { // room was SHARED before
                            liveQuizId: liveQuizId,
                            currentQuestionIndex: currentQuiz.currentQuestionIndex, 
                            question: currentQuiz.quiz.questions[currentQuiz.currentQuestionIndex]
                        });
        ...
        socket.on(START_QUIZ_MESSAGE, function(data) {
            ...
            then(function (quiz) {
                if (quiz) {
                    ...
                    io.sockets.in(liveQuizId).emit(START_QUIZ_OK_MESSAGE, { // room was SHARED before
                        liveQuizId: liveQuizId, 
                        currentQuestionIndex: 0, 
        ...
        socket.on(STOP_QUIZ_MESSAGE, function(data) {
            ...
            // Send the STOP QUIZ to all
            io.sockets.in(liveQuizId).emit(STOP_QUIZ_OK_MESSAGE, {liveQuizId: liveQuizId}); // room was SAHRED before
        });
        ...
        socket.on('message', function(data) {
            ...
            io.sockets.in(SHARED_ROOM).emit('message', message);
        });
